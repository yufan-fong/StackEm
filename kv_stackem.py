...
class GameWidget(Widget):
    ...
    next_block = Block(start_x,start_y)
    ...
    def move_step(self,dt):
        top_block = len(tower)-1
        landing_line = tower[top_block].y
        if self.drop == True and self.next_block.y > landing_line-self.next_block.size[1]:
            self.next_block.y -= 10*dt
            print('here')
            if self.next_block.y == landing_line-self.next_block.size[1]:
                self.drop = False
